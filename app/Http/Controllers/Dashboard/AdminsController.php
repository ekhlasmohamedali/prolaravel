...
class AdminsController extends Controller
{
    ...
    {
        $count = Admin::whereDoesntHaveRole()->orWhereRoleIs(['manager'])->count();
        $admins = Admin::where('id', '<>', auth()->user()->id)->whereDoesntHaveRole()
                        ->orWhereRoleIs(['manager'])
                        ->where($request->column ?? 'id', 'like', '%' . $request->search . '%')->latest()->paginate(5);
        return view('dashboard.admins.index', compact('admins', 'count'));
    }
